...
use std::io::{self, Read, BufRead, BufWriter, Write};
        ...
        match words[0] {
            ...
            "calc" => 'a: loop {
                let mut value_total: i16 = match hashmap.get(words[1]) {
                    Some(v) => *v,
                    None => {
                        writeln!(output, "{} unknown", &line[5..line.len()]);
                        break 'a;
                    }
                };
                let x = words.len() - 1;
                ...
                writeln!(output, "{} {}", &line[5..line.len()], v);
                break 'a;
            },
            "clear" => {
                hashmap.clear();
...
fn dicecup() {
    ...
    let mut numbers = String::with_capacity(6);
    io::stdin().read_line(&mut numbers).unwrap();
    let numbers: Vec<u8> = numbers.split_whitespace().map(|n| n.parse().unwrap())
        .collect();

    if numbers[0] < numbers[1] {
        for i in numbers[0] + 1 .. numbers[1] + 2 {
            writeln!(output, "{}", i);
        }
    } else {
        for i in numbers[1] + 1 .. numbers[0] + 2 {
            writeln!(output, "{}", i);
        }
    }
}
...
fn sevenwonders() {
    let mut line = String::with_capacity(51);
    ...
    println!("{}", array.iter().min().unwrap() * 7 + array[0].pow(2) + array[1].pow(2) + array[2].pow(2));
}